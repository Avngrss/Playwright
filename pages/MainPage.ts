...
export class MainPage extends BasePage {
    ...
    async setRange(minValue: number, maxValue: number): Promise<void> {
        await this.page.evaluate(({ minSelector, minValue, maxSelector, maxValue }) => {
            ...
            minSlider.style.left = `${(minValue / 200) * 100}%`; // Предполагается, что максимальное значение ползунка 200
            ...
            maxSlider.style.left = `${(maxValue / 200) * 100}%`; // Предполагается, что максимальное значение ползунка 200
        ...
        // Подождать, пока хотя бы один товар загрузится или появится текст "There are no products found"
        await Promise.race([
